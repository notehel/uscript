...
type
  ...
  public
    ...
    function DoString(content: String): string; virtual;
    // load file and execute
    procedure RegisterFunction(FuncName: AnsiString;
      MethodName: AnsiString = ''; Obj: TObject = NIL); virtual;
    // register function
    procedure AutoRegisterFunctions(Obj: TObject);
    // register all published functions
    procedure UnregisterFunctions(Obj: TObject);
    // unregister all object functions
...
type
  ...
  //
function LuaCallBack(L: {$IFNDEF LUA52} Lua_State {$ELSE} Plua_State
  {$ENDIF}): Integer; cdecl;
var
...
procedure LuaHook(L: Plua_State; ar: Plua_Debug); cdecl;
var
  msgs: Tbytes;
  i:integer;
  outText:string;
begin
  ...
  if ar.event = LUA_MASKLINE then
  begin

  end
  else if ar.event = LUA_MASKCALL then
  begin

  end
  else if ar.event = LUA_MASKRET then
  begin

  end;

  setlength(msgs, length(ar.short_src));

  for I := 0 to length(ar.short_src) - 1 do
  begin
    msgs[i] := byte(ar.short_src[i]);
  end;

  outText := Tencoding.Default.GetString(msgs);

  showmessage(outText);

end;

function TLua.DoString(content: String): string;
var
  debug: Plua_Debug;
  level: Integer;
  errMsg : string;
begin
  //lua_sethook(LuaInstance, LuaHook, LUA_MASKLINE or LUA_MASKCALL or LUA_MASKRET, 0);

  luaL_openlibs(LuaInstance);
  luaL_dostring(LuaInstance, PAnsiChar(AnsiString(content)));
  ...
  // New(debug);
  // level := 1;
  // while lua_getstack(LuaInstance, level, @debug) = 1 do
  // begin
  // inc(level);
  // end;
  //
  // lua_getstack(LuaInstance, level, @debug);
  //
  // Dispose(debug);
...
//
procedure TLua.RegisterFunction(FuncName: AnsiString;
  MethodName: AnsiString = ''; Obj: TObject = NIL);
var
...
begin
  ...
  for I := 0 to MethodTable^.Count - 1 do
    RegisterFunction(MethodTable^.Entries[I].Name^,
      MethodTable^.Entries[I].Name^, Obj);
end;
{$ENDIF}
